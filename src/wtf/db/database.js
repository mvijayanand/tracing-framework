...
wtf.db.Database = function(opt_useStorage) {
   ...
   * @private
   */
  this.eventTable_ = new wtf.db.EventTypeTable();
...
wtf.db.Database.prototype.getEventType = function(name) {
  return this.eventTable_.getByName(name);
};
 ...
 */
wtf.db.Database.prototype.query = function(expr) {
  ...
  if (isFilter) {
    ...
    for (var n = 0; n < this.zoneList_.length; n++) {
      var zone = this.zoneList_[n];
      // zone.forEach(Number.MIN_VALUE, Number.MAX_VALUE, function(e) {
...
wtf.db.Database.prototype.endInsertingEvents = function() {
  ...
  //goog.global.console.log('db rebuild', rebuildEndTime - insertEndTime);
...
goog.exportProperty(
    ...
    wtf.db.Database.prototype.getLastEventTime);
goog.exportProperty(
    wtf.db.Database.prototype, 'query',
    wtf.db.Database.prototype.query);
